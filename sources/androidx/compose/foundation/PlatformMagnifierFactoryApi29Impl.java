...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl mo191createnHHXs2Y(View view, boolean useTextDefault, long size, float cornerRadius, float elevation, boolean clippingEnabled, Density density, float initialZoom) {
        if (useTextDefault) {
            return new PlatformMagnifierImpl(new Magnifier(view));
        }
        long jMo255toSizeXkaWNTQ = density.mo255toSizeXkaWNTQ(size);
        float fMo254toPx0680j_4 = density.mo254toPx0680j_4(cornerRadius);
        float fMo254toPx0680j_42 = density.mo254toPx0680j_4(elevation);
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (jMo255toSizeXkaWNTQ != 9205357640488583168L) {
            builder.setSize(MathKt.roundToInt(Size.m878getWidthimpl(jMo255toSizeXkaWNTQ)), MathKt.roundToInt(Size.m876getHeightimpl(jMo255toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(fMo254toPx0680j_4)) {
            builder.setCornerRadius(fMo254toPx0680j_4);
        }
        if (!Float.isNaN(fMo254toPx0680j_42)) {
            builder.setElevation(fMo254toPx0680j_42);
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo190updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m856isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m842getXimpl(sourceCenter), Offset.m843getYimpl(sourceCenter), Offset.m842getXimpl(magnifierCenter), Offset.m843getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m842getXimpl(sourceCenter), Offset.m843getYimpl(sourceCenter));
            }
        }
    }
}
