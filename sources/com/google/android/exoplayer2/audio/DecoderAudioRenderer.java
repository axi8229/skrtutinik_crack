...
public abstract class DecoderAudioRenderer extends BaseRenderer implements MediaClock {
    ...
    protected abstract Decoder<DecoderInputBuffer, ? extends SimpleOutputBuffer, ? extends DecoderException> createDecoder(Format format, ExoMediaCrypto mediaCrypto) throws DecoderException;
    ...
    private boolean drainOutputBuffer() throws Exception {
        if (this.outputBuffer == null) {
            SimpleOutputBuffer simpleOutputBufferDequeueOutputBuffer = this.decoder.dequeueOutputBuffer();
            this.outputBuffer = simpleOutputBufferDequeueOutputBuffer;
            if (simpleOutputBufferDequeueOutputBuffer == null) {
                return false;
            }
            int i = simpleOutputBufferDequeueOutputBuffer.skippedOutputBufferCount;
            if (i > 0) {
        ...
        if (!audioSink.handleBuffer(simpleOutputBuffer.data, simpleOutputBuffer.timeUs, 1)) {
            return false;
        }
    ...
    private boolean feedInputBuffer() throws Exception {
        Decoder<DecoderInputBuffer, ? extends SimpleOutputBuffer, ? extends DecoderException> decoder = this.decoder;
    ...
    private final class AudioSinkListener implements AudioSink.Listener {
        ...
        public void onAudioSessionId(int audioSessionId) {
            DecoderAudioRenderer.this.eventDispatcher.audioSessionId(audioSessionId);
        ...
        public void onUnderrun(int bufferSize, long bufferSizeMs, long elapsedSinceLastFeedMs) {
            DecoderAudioRenderer.this.eventDispatcher.audioTrackUnderrun(bufferSize, bufferSizeMs, elapsedSinceLastFeedMs);
            DecoderAudioRenderer.this.onAudioTrackUnderrun(bufferSize, bufferSizeMs, elapsedSinceLastFeedMs);
        }

        @Override // com.google.android.exoplayer2.audio.AudioSink.Listener
        public void onSkipSilenceEnabledChanged(boolean skipSilenceEnabled) {
            DecoderAudioRenderer.this.eventDispatcher.skipSilenceEnabledChanged(skipSilenceEnabled);
            DecoderAudioRenderer.this.onAudioTrackSkipSilenceEnabledChanged(skipSilenceEnabled);
        }
    }
}
