...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    public T sizeMultiplier(float f) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().sizeMultiplier(f);
        }
        if (f < 0.0f || f > 1.0f) {
    ...
    public T useAnimationPool(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().useAnimationPool(z);
        }
    ...
    public T diskCacheStrategy(DiskCacheStrategy diskCacheStrategy) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().diskCacheStrategy(diskCacheStrategy);
        }
    ...
    public T priority(Priority priority) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().priority(priority);
        }
    ...
    public T placeholder(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().placeholder(drawable);
        }
    ...
    public T placeholder(int i) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().placeholder(i);
        }
    ...
    public T skipMemoryCache(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().skipMemoryCache(true);
        }
        this.isCacheable = !z;
        this.fields |= 256;
        return (T) selfOrThrowIfLocked();
    ...
    public T override(int i, int i2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().override(i, i2);
        }
    ...
    public T signature(Key key) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().signature(key);
        }
    ...
    public <Y> T set(Option<Y> option, Y y) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().set(option, y);
        }
    ...
    public T decode(Class<?> cls) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().decode(cls);
        }
    ...
    final T optionalTransform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().optionalTransform(downsampleStrategy, transformation);
        }
    ...
    final T transform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().transform(downsampleStrategy, transformation);
        }
        downsample(downsampleStrategy);
    ...
    T transform(Transformation<Bitmap> transformation, boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().transform(transformation, z);
        }
    ...
    <Y> T transform(Class<Y> cls, Transformation<Y> transformation, boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().transform(cls, transformation, z);
        }
    ...
    public T dontTransform() {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().dontTransform();
        }
        this.transformations.clear();
        int i = this.fields;
        this.isTransformationRequired = false;
        this.isTransformationAllowed = false;
        this.fields = (i & (-133121)) | pj_ssl_sock_proto.PJ_SSL_SOCK_PROTO_DTLS1;
    ...
    public T apply(BaseRequestOptions<?> baseRequestOptions) {
        if (this.isAutoCloneEnabled) {
            return (T) mo2307clone().apply(baseRequestOptions);
        }
        ...
        if (isSet(baseRequestOptions.fields, pjmedia_tp_proto.PJMEDIA_TP_PROFILE_AVP)) {
            this.theme = baseRequestOptions.theme;
        }
        if (isSet(baseRequestOptions.fields, pj_ssl_sock_proto.PJ_SSL_SOCK_PROTO_DTLS1)) {
