...
public final class ContentInViewNode extends Modifier.Node implements BringIntoViewResponder, LayoutAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    private long viewportSize = IntSize.INSTANCE.m2160getZeroYbymL2g();
    ...
    public Rect calculateRectForParent(Rect localRect) {
        if (IntSize.m2154equalsimpl0(this.viewportSize, IntSize.INSTANCE.m2160getZeroYbymL2g())) {
            throw new IllegalStateException("Expected BringIntoViewRequester to not be used before parents are placed.");
        }
        return m215computeDestinationO0kMr_c(localRect, this.viewportSize);
    }
    ...
    public Object bringChildIntoView(Function0<Rect> function0, Continuation<? super Unit> continuation) {
        ...
        if (rectInvoke == null || m217isMaxVisibleO0kMr_c$default(this, rectInvoke, 0L, 1, null)) {
    ...
    public void mo220onRemeasuredozmzZPI(long size) {
        ...
        if (m213compareToTemP2vQ(size, j) < 0 && (focusedChildBounds = getFocusedChildBounds()) != null) {
            Rect rect = this.focusedChildBoundsFromPreviousRemeasure;
            if (rect == null) {
                rect = focusedChildBounds;
            }
            if (!this.isAnimationRunning && !this.trackingFocusedChild && m216isMaxVisibleO0kMr_c(rect, j) && !m216isMaxVisibleO0kMr_c(focusedChildBounds, size)) {
                ...
                if (i == 0) {
                    ...
                    Function1<Float, Unit> function1 = new Function1<Float, Unit>() { // from class: androidx.compose.foundation.gestures.ContentInViewNode.launchAnimation.2.1.1
                        ...
                        public final void invoke(float f) {
                            ...
                            float fM276toFloatk4lQ0M = f2 * scrollingLogic.m276toFloatk4lQ0M(scrollingLogic.m274reverseIfNeededMKHz9U(nestedScrollScope.mo246scrollByOzD1aCk(scrollingLogic.m274reverseIfNeededMKHz9U(scrollingLogic.m277toOffsettuRUvjQ(f2 * f)), NestedScrollSource.INSTANCE.m1313getUserInputWNlRxjI())));
                            if (Math.abs(fM276toFloatk4lQ0M) < Math.abs(f)) {
                                JobKt__JobKt.cancel$default(job, "Scroll animation cancelled because scroll was not consumed (" + fM276toFloatk4lQ0M + " < " + f + ')', null, 2, null);
                            }
                        }
                    };
                    ...
                    Function0<Unit> function0 = new Function0<Unit>() { // from class: androidx.compose.foundation.gestures.ContentInViewNode.launchAnimation.2.1.2
                        ...
                        public final void invoke2() {
                            ...
                            while (true) {
                                ...
                                if (!(rectInvoke == null ? true : ContentInViewNode.m217isMaxVisibleO0kMr_c$default(contentInViewNode3, rectInvoke, 0L, 1, null))) {
                                    break;
                                } else {
                                    ((Request) bringIntoViewRequestPriorityQueue.requests.removeAt(bringIntoViewRequestPriorityQueue.requests.getSize() - 1)).getContinuation().resumeWith(Result.m2628constructorimpl(Unit.INSTANCE));
                                }
                            }
                            if (contentInViewNode2.trackingFocusedChild) {
                                Rect focusedChildBounds = contentInViewNode2.getFocusedChildBounds();
                                if (focusedChildBounds != null && ContentInViewNode.m217isMaxVisibleO0kMr_c$default(contentInViewNode2, focusedChildBounds, 0L, 1, null)) {
    ...
    public final float calculateScrollDelta(BringIntoViewSpec bringIntoViewSpec) {
        ...
        long jM2162toSizeozmzZPI = IntSizeKt.m2162toSizeozmzZPI(this.viewportSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return bringIntoViewSpec.calculateScrollDistance(rectFindBringIntoViewRequest.getTop(), rectFindBringIntoViewRequest.getBottom() - rectFindBringIntoViewRequest.getTop(), Size.m876getHeightimpl(jM2162toSizeozmzZPI));
        }
        if (i == 2) {
            return bringIntoViewSpec.calculateScrollDistance(rectFindBringIntoViewRequest.getLeft(), rectFindBringIntoViewRequest.getRight() - rectFindBringIntoViewRequest.getLeft(), Size.m878getWidthimpl(jM2162toSizeozmzZPI));
        }
    ...
    private final Rect findBringIntoViewRequest() {
        ...
        if (size > 0) {
            ...
            do {
                ...
                if (rectInvoke != null) {
                    if (m214compareToiLBOSCw(rectInvoke.m861getSizeNHjbRc(), IntSizeKt.m2162toSizeozmzZPI(this.viewportSize)) > 0) {
    ...
    private final Rect m215computeDestinationO0kMr_c(Rect childBounds, long containerSize) {
        return childBounds.m863translatek4lQ0M(Offset.m850unaryMinusF1C5BW0(m218relocationOffsetBMxPBkI(childBounds, containerSize)));
    }

    /* renamed from: isMaxVisible-O0kMr_c$default, reason: not valid java name */
    static /* synthetic */ boolean m217isMaxVisibleO0kMr_c$default(ContentInViewNode contentInViewNode, Rect rect, long j, int i, Object obj) {
        if ((i & 1) != 0) {
            j = contentInViewNode.viewportSize;
        }
        return contentInViewNode.m216isMaxVisibleO0kMr_c(rect, j);
    }

    /* renamed from: isMaxVisible-O0kMr_c, reason: not valid java name */
    private final boolean m216isMaxVisibleO0kMr_c(Rect rect, long j) {
        long jM218relocationOffsetBMxPBkI = m218relocationOffsetBMxPBkI(rect, j);
        return Math.abs(Offset.m842getXimpl(jM218relocationOffsetBMxPBkI)) <= 0.5f && Math.abs(Offset.m843getYimpl(jM218relocationOffsetBMxPBkI)) <= 0.5f;
    }

    /* renamed from: relocationOffset-BMxPBkI, reason: not valid java name */
    private final long m218relocationOffsetBMxPBkI(Rect childBounds, long containerSize) {
        long jM2162toSizeozmzZPI = IntSizeKt.m2162toSizeozmzZPI(containerSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return OffsetKt.Offset(0.0f, requireBringIntoViewSpec().calculateScrollDistance(childBounds.getTop(), childBounds.getBottom() - childBounds.getTop(), Size.m876getHeightimpl(jM2162toSizeozmzZPI)));
        }
        if (i == 2) {
            return OffsetKt.Offset(requireBringIntoViewSpec().calculateScrollDistance(childBounds.getLeft(), childBounds.getRight() - childBounds.getLeft(), Size.m878getWidthimpl(jM2162toSizeozmzZPI)), 0.0f);
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-TemP2vQ, reason: not valid java name */
    private final int m213compareToTemP2vQ(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Intrinsics.compare(IntSize.m2155getHeightimpl(j), IntSize.m2155getHeightimpl(j2));
        }
        if (i == 2) {
            return Intrinsics.compare(IntSize.m2156getWidthimpl(j), IntSize.m2156getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-iLBOSCw, reason: not valid java name */
    private final int m214compareToiLBOSCw(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Float.compare(Size.m876getHeightimpl(j), Size.m876getHeightimpl(j2));
        }
        if (i == 2) {
            return Float.compare(Size.m878getWidthimpl(j), Size.m878getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }
