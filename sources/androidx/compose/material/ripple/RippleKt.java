...
public final class RippleKt {
    ...
    public static final Indication m698rememberRipple9IZ8Weo(boolean z, float f, long j, Composer composer, int i, int i2) {
        ...
        if ((i2 & 4) != 0) {
            j = Color.INSTANCE.m990getUnspecified0d7_KjU();
        }
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventStart(1635163520, i, -1, "androidx.compose.material.ripple.rememberRipple (Ripple.kt:80)");
        }
        State stateRememberUpdatedState = SnapshotStateKt.rememberUpdatedState(Color.m971boximpl(j), composer, (i >> 6) & 14);
        Boolean boolValueOf = Boolean.valueOf(z);
        Dp dpM2100boximpl = Dp.m2100boximpl(f);
        composer.startReplaceableGroup(511388516);
        boolean zChanged = composer.changed(boolValueOf) | composer.changed(dpM2100boximpl);
    ...
    public static final AnimationSpec<Float> outgoingStateLayerAnimationSpecFor(Interaction interaction) {
        if (!(interaction instanceof HoverInteraction$Enter) && !(interaction instanceof FocusInteraction$Focus)) {
            return interaction instanceof DragInteraction$Start ? new TweenSpec(TimelineAnimator.DURATION, 0, EasingKt.getLinearEasing(), 2, null) : DefaultTweenSpec;
        }
        return DefaultTweenSpec;
    }
}
