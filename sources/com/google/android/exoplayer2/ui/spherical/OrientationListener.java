...
final class OrientationListener implements SensorEventListener {
    ...
    private float extractRoll(float[] matrix) {
        SensorManager.remapCoordinateSystem(matrix, 1, pjsip_transport_type_e.PJSIP_TRANSPORT_TLS6, this.tempMatrix4x4);
    ...
    private void rotateAroundZ(float[] matrix, int rotation) {
        if (rotation != 0) {
            int i = pjsip_transport_type_e.PJSIP_TRANSPORT_UDP6;
            int i2 = 1;
            if (rotation == 1) {
                i2 = 129;
                i = 2;
