...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> state, State<RippleAlpha> state2, ViewGroup viewGroup) {
        ...
        this.rippleSize = Size.INSTANCE.m884getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int iMo248roundToPx0680j_4;
        this.rippleSize = contentDrawScope.mo1176getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            iMo248roundToPx0680j_4 = MathKt.roundToInt(RippleAnimationKt.m692getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo1176getSizeNHjbRc()));
        } else {
            iMo248roundToPx0680j_4 = contentDrawScope.mo248roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = iMo248roundToPx0680j_4;
        long value = this.color.getValue().getValue();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        m697drawStateLayerH2RKhps(contentDrawScope, this.radius, value);
        Canvas canvas = contentDrawScope.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m696updateRipplePropertiesbiQXAtU(contentDrawScope.mo1176getSizeNHjbRc(), this.rippleRadius, value, pressedAlpha);
    ...
    public void addRipple(PressInteraction$Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m695addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().getValue(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
