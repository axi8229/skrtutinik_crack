...
public final class DefaultExtractorInput implements ExtractorInput {
    ...
    private byte[] peekBuffer = new byte[pj_ssl_sock_proto.PJ_SSL_SOCK_PROTO_DTLS1];
    ...
    public int read(byte[] target, int offset, int length) throws IOException {
        ...
        if (fromPeekBuffer == 0) {
            fromPeekBuffer = readFromUpstream(target, offset, length, 0, true);
        }
    ...
    private void ensureSpaceForPeek(int length) {
        ...
        if (i > bArr.length) {
            this.peekBuffer = Arrays.copyOf(this.peekBuffer, Util.constrainValue(bArr.length * 2, pj_ssl_sock_proto.PJ_SSL_SOCK_PROTO_DTLS1 + i, i + 524288));
        }
    }
    ...
    private void updatePeekBuffer(int bytesConsumed) {
        ...
        this.peekBufferPosition = 0;
        byte[] bArr = this.peekBuffer;
        byte[] bArr2 = i < bArr.length - 524288 ? new byte[pj_ssl_sock_proto.PJ_SSL_SOCK_PROTO_DTLS1 + i] : bArr;
