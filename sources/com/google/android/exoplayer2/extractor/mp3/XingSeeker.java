...
final class XingSeeker implements Seeker {
    ...
    public static XingSeeker create(long inputLength, long position, MpegAudioUtil.Header mpegAudioHeader, ParsableByteArray frame) {
        ...
        long unsignedIntToInt2 = frame.readUnsignedIntToInt();
        long[] jArr = new long[100];
        for (int i4 = 0; i4 < 100; i4++) {
            jArr[i4] = frame.readUnsignedByte();
        }
        if (inputLength != -1) {
            long j = position + unsignedIntToInt2;
            if (inputLength != j) {
                Log.w("XingSeeker", "XING data size mismatch: " + inputLength + ", " + j);
            }
        }
        return new XingSeeker(position, mpegAudioHeader.frameSize, jScaleLargeTimestamp, unsignedIntToInt2, jArr);
    }
    ...
    public long getTimeUs(long position) {
        ...
        return timeUsForTableIndex + Math.round((j2 == (iBinarySearchFloor == 99 ? 256L : jArr[i]) ? ConfigValue.DOUBLE_DEFAULT_VALUE : (d - j2) / (r0 - j2)) * (timeUsForTableIndex2 - timeUsForTableIndex));
    }
