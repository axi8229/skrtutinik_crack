...
public final class Mp3Extractor implements Extractor {
    ...
    private int readSample(ExtractorInput extractorInput) throws IOException {
        ...
        int iSampleData = this.currentTrackOutput.sampleData(extractorInput, this.sampleBytesRemaining, true);
        if (iSampleData == -1) {
            return -1;
        }
    ...
    private Seeker computeSeeker(ExtractorInput input) throws IOException {
        ...
        if ((this.flags & 2) != 0) {
            ...
            seekerMaybeReadSeekFrame = new IndexSeeker(durationUs, input.getPosition(), dataEndPosition);
    ...
    private Seeker maybeReadSeekFrame(ExtractorInput input) throws IOException {
        ...
        input.peekFully(parsableByteArray.data, 0, this.synchronizedHeader.frameSize);
        ...
        if (xingSeekerCreate != null && !this.gaplessInfoHolder.hasGaplessInfo()) {
            input.resetPeekPosition();
            input.advancePeekPosition(i4 + 141);
            input.peekFully(this.scratch.data, 0, 3);
            this.scratch.setPosition(0);
            this.gaplessInfoHolder.setFromXingHeaderValue(this.scratch.readUnsignedInt24());
        }
        input.skipFully(this.synchronizedHeader.frameSize);
        return (xingSeekerCreate == null || xingSeekerCreate.isSeekable() || seekFrameHeader != 1231971951) ? xingSeekerCreate : getConstantBitrateSeeker(input);
    }
