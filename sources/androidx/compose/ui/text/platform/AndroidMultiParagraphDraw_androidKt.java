...
public final class AndroidMultiParagraphDraw_androidKt {
    ...
    public static final void m1942drawMultiParagraph7AXcY_I(MultiParagraph multiParagraph, Canvas canvas, Brush brush, float f, Shadow shadow, TextDecoration textDecoration, DrawStyle drawStyle, int i) {
        canvas.save();
        if (multiParagraph.getParagraphInfoList$ui_text_release().size() <= 1 || (brush instanceof SolidColor)) {
            m1943drawParagraphs7AXcY_I(multiParagraph, canvas, brush, f, shadow, textDecoration, drawStyle, i);
            ...
            Shader shaderMo962createShaderuvyYCjk = ((ShaderBrush) brush).mo962createShaderuvyYCjk(SizeKt.Size(fMax, height));
            Matrix matrix = new Matrix();
            shaderMo962createShaderuvyYCjk.getLocalMatrix(matrix);
            ...
            for (int i3 = 0; i3 < size2; i3++) {
                ...
                paragraphInfo2.getParagraph().mo1685painthn5TExg(canvas, BrushKt.ShaderBrush(shaderMo962createShaderuvyYCjk), f, shadow, textDecoration, drawStyle, i);
                ...
                shaderMo962createShaderuvyYCjk.setLocalMatrix(matrix);
            }
        }
        canvas.restore();
    }
    ...
    private static final void m1943drawParagraphs7AXcY_I(MultiParagraph multiParagraph, Canvas canvas, Brush brush, float f, Shadow shadow, TextDecoration textDecoration, DrawStyle drawStyle, int i) {
        List<ParagraphInfo> paragraphInfoList$ui_text_release = multiParagraph.getParagraphInfoList$ui_text_release();
        int size = paragraphInfoList$ui_text_release.size();
        for (int i2 = 0; i2 < size; i2++) {
            ParagraphInfo paragraphInfo = paragraphInfoList$ui_text_release.get(i2);
            paragraphInfo.getParagraph().mo1685painthn5TExg(canvas, brush, f, shadow, textDecoration, drawStyle, i);
