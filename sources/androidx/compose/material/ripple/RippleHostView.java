...
public final class RippleHostView extends View {
    ...
    public final void m695addRippleKOepWvA(PressInteraction$Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m696updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            unprojectedRipple.setHotspot(Offset.m842getXimpl(interaction.getPressPosition()), Offset.m843getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m696updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple unprojectedRipple = this.ripple;
        if (unprojectedRipple == null) {
            return;
        }
        unprojectedRipple.trySetRadius(radius);
        unprojectedRipple.m703setColorDxMtmZc(color, alpha);
        Rect rect = new Rect(0, 0, MathKt.roundToInt(Size.m878getWidthimpl(size)), MathKt.roundToInt(Size.m876getHeightimpl(size)));
        setLeft(rect.left);
        setTop(rect.top);
        setRight(rect.right);
