...
public final class Huffman {
    ...
    public final void encode(ByteString source, BufferedSink sink) throws IOException {
        ...
        while (i < size) {
            int i3 = i + 1;
            int iAnd = Util.and(source.getByte(i), KotlinVersion.MAX_COMPONENT_VALUE);
            int i4 = CODES[iAnd];
    ...
    public final int encodedLength(ByteString bytes) {
        ...
        for (int i = 0; i < bytes.size(); i++) {
            j += CODE_BIT_COUNTS[Util.and(bytes.getByte(i), KotlinVersion.MAX_COMPONENT_VALUE)];
        }
        return (int) ((j + 7) >> 3);
    }
    ...
    public final void decode(BufferedSource source, long byteCount, BufferedSink sink) {
        ...
        while (j < byteCount) {
            ...
            while (terminalBitCount >= 8) {
                int i = (iAnd >>> (terminalBitCount - 8)) & KotlinVersion.MAX_COMPONENT_VALUE;
        ...
        while (terminalBitCount > 0) {
            int i2 = (iAnd << (8 - terminalBitCount)) & KotlinVersion.MAX_COMPONENT_VALUE;
