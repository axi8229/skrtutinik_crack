...
final class MergingMediaPeriod implements MediaPeriod, MediaPeriod.Callback {
    ...
    public MergingMediaPeriod(CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory, long[] periodTimeOffsetsUs, MediaPeriod... periods) {
        ...
        for (int i = 0; i < periods.length; i++) {
            long j = periodTimeOffsetsUs[i];
            if (j != 0) {
                this.periods[i] = new TimeOffsetMediaPeriod(periods[i], j);
            }
        }
    }
    ...
    public long selectTracks(TrackSelection[] selections, boolean[] mayRetainStreamFlags, SampleStream[] streams, boolean[] streamResetFlags, long positionUs) {
        ...
        SampleStream[] sampleStreamArr3 = new SampleStream[selections.length];
        TrackSelection[] trackSelectionArr = new TrackSelection[selections.length];
        ...
        while (i3 < this.periods.length) {
            for (int i4 = 0; i4 < selections.length; i4++) {
                ...
                if (iArr2[i4] == i3) {
                    trackSelection2 = selections[i4];
                }
                trackSelectionArr[i4] = trackSelection2;
            }
            int i5 = i3;
            TrackSelection[] trackSelectionArr2 = trackSelectionArr;
            ArrayList arrayList2 = arrayList;
            long jSelectTracks = this.periods[i3].selectTracks(trackSelectionArr, mayRetainStreamFlags, sampleStreamArr3, streamResetFlags, j);
            if (i5 == 0) {
                j = jSelectTracks;
    ...
    private static final class TimeOffsetSampleStream implements SampleStream {
        ...
        public int readData(FormatHolder formatHolder, DecoderInputBuffer buffer, boolean formatRequired) {
            int data = this.sampleStream.readData(formatHolder, buffer, formatRequired);
            if (data == -4) {
